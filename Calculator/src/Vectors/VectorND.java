...
public class VectorND  {
	...
	public VectorND add (VectorND vector) {
		...
		return new VectorND(Coordinats);
	}
    ...
    public String toString() {
		 ...
		 for (int i = 0; i< this.Coordinats.length; i++) {
			 if (i< this.Coordinats.length - 1) vector = vector+ this.Coordinats[i].toString() + ", ";
    ...
    public boolean equals(Object obj) {
		if (this == obj) return true;
 		...
 		if (vector.Coordinats.length != this.Coordinats.length) return false;
 		for (int i = 0; i< this.Coordinats.length; i++)
